...
class Session
{
	...
	{
		$obj = (object) array("user_id" => -1, "user_email" => "", "user_account_type" => -1); // new stdClass();
		if (isset($session_id)) {
			$data = self::SessionData($session_id, true);
			if (isset($data->user_logged_in)) {
				// we assume the rest is also built
				$obj->user_id = $data->user_id;
				// $obj->user_name = $data->user_email;
